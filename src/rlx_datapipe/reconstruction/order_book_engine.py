...
class GapStatistics:
    ...
    total_gaps: int = 0
    max_gap_size: int = 0
    last_gap_update_id: int | None = None
...
class OrderBookEngine:
    ...
    def process_snapshot(self, snapshot_data: pl.DataFrame) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Processed snapshot #{self.snapshot_count} with update_id={self.last_update_id}"
            )
    ...
    def calculate_drift(self, snapshot_data: pl.DataFrame) -> dict[str, float]:
        ...
        try:
            ...
            snapshot_bids = snapshot_data.filter(pl.col("side") == "BID").sort(
            ...
            for i in range(min(len(current_bids), len(snapshot_bids))):
                ...
                bid_quantity_diff += abs(
                    current_bids[i][1] - snapshot_bids["quantity"][i]
                )
            ...
            for i in range(min(len(current_asks), len(snapshot_asks))):
                ...
                ask_quantity_diff += abs(
                    current_asks[i][1] - snapshot_asks["quantity"][i]
                )
    ...
    def get_checkpoint_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_update_id": self.last_update_id,
            "gap_stats": (
                self.gap_stats.__dict__
                if hasattr(self.gap_stats, "__dict__")
                else self.gap_stats
            ),
            "updates_processed": self.updates_processed,
            "snapshot_count": self.snapshot_count,
            "drift_metrics": self.drift_metrics,
            "processing_rate": self.updates_processed
            / max(1, time.time() - self.last_gc_time),
        }
