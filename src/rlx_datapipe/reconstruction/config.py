...
class ReplayOptimizationConfig:
    ...
    def __post_init__(self):
        ...
        if self.drift_threshold <= 0 or self.drift_threshold > 1:
            raise ValueError(
                f"drift_threshold must be between 0 and 1, got {self.drift_threshold}"
            )

        if self.drift_check_interval < 1:
            raise ValueError(
                f"drift_check_interval must be positive, got {self.drift_check_interval}"
            )

        if self.max_book_levels < 1 or self.max_book_levels > 1000:
            raise ValueError(
                f"max_book_levels must be between 1 and 1000, got {self.max_book_levels}"
            )
        ...
        if self.gc_interval < 1000:
            raise ValueError(
                f"gc_interval must be at least 1000, got {self.gc_interval}"
            )

        if self.max_memory_mb < 100 or self.max_memory_mb > 10000:
            raise ValueError(
                f"max_memory_mb must be between 100 and 10000, got {self.max_memory_mb}"
            )

        if self.profile_interval < 100:
            raise ValueError(
                f"profile_interval must be at least 100, got {self.profile_interval}"
            )
...
class MultiSymbolConfig:
    ...
    def from_dict(cls, config_dict: dict[str, Any]) -> "MultiSymbolConfig":
        ...
        routing_strategy = RoutingStrategy(
            config_dict.get("routing_strategy", "direct")
        )
