...
class DeltaFeedProcessor:
    ...
    def __init__(
        ...
        self.expected_next_id: int | None = None
    ...
    def validate_and_sort(
        ...
        try:
            ...
            if enforce_monotonic:
                origin_times = (
                    sorted_batch.get_column("origin_time")
                    if "origin_time" in sorted_batch.columns
                    else None
                )
                ...
                if gaps_found:
                    logger.warning(f"Found {len(gaps_found)} sequence gaps in batch")
    ...
    def _record_gap(self, gap_info: SequenceGapInfo) -> None:
        ...
        if gap_size > self.gap_threshold:
            ...
            logger.error(f"Large gap detected: {gap_info}. Recovery signaled.")
    ...
    def process_batch(
        ...
        if gaps:
            ...
            sorted_batch = sorted_batch.with_columns([is_after_gap.alias("after_gap")])
        else:
            sorted_batch = sorted_batch.with_columns([pl.lit(False).alias("after_gap")])
