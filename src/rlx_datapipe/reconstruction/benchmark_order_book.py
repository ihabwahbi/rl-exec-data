...
class OrderBookBenchmark:
    ...
    def generate_test_data(
        ...
        delta_data = {
            ...
            "new_quantity": np.random.uniform(
                quantity_range[0], quantity_range[1], num_deltas
            ),
        ...
        for i in range(num_snapshots):
            ...
            bid_prices = np.linspace(
                price_range[0], np.mean(price_range) - 10, snapshot_levels
            )
            bid_quantities = np.random.uniform(
                quantity_range[0], quantity_range[1], snapshot_levels
            )

            # Generate ask levels
            ask_prices = np.linspace(
                np.mean(price_range) + 10, price_range[1], snapshot_levels
            )
            ...
            snapshot_data = {
                "origin_time": [i * num_deltas // num_snapshots * 1000]
                * (2 * snapshot_levels),
                "update_id": [1000 + i * num_deltas // num_snapshots]
                * (2 * snapshot_levels),
    ...
    def benchmark_basic_engine(
        ...
        deltas_df = test_data["deltas"].with_columns(
            [
                test_data["deltas"]["price"]
                .map_elements(
                    lambda x: float_to_scaled_int64(x),
                    return_dtype=pl.Int64,
                )
                .alias("price"),
                test_data["deltas"]["new_quantity"]
                .map_elements(
                    lambda x: float_to_scaled_int64(x),
                    return_dtype=pl.Int64,
                )
