...
class ChronologicalEventReplay:
    ...
    def _sort_events(self, events_df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "update_id" not in events_df.columns:
            events_df = events_df.with_columns(pl.lit(0).alias("update_id"))

        # Stable sort maintaining original order for equal timestamps
        sorted_df = events_df.sort(
            ["origin_time", "update_id"], descending=[False, False], maintain_order=True
        )
    ...
    def _label_event_types(self, events_df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "trade_id" in events_df.columns:
            conditions.append(
                pl.when(pl.col("trade_id").is_not_null()).then(pl.lit("TRADE"))
            )

        # Snapshot events have is_snapshot flag or both bids and asks
        if "is_snapshot" in events_df.columns:
            conditions.append(
                pl.when(pl.col("is_snapshot") == True).then(pl.lit("BOOK_SNAPSHOT"))
            )
        elif "bids" in events_df.columns and "asks" in events_df.columns:
            conditions.append(
                pl.when(
                    pl.col("bids").is_not_null() & pl.col("asks").is_not_null()
        ...
        return events_df.with_columns(expr.alias("event_type"))
    ...
    def _stateful_replay(self, events_df: pl.DataFrame) -> pl.DataFrame:
        ...
        for row in events_df.iter_rows(named=True):
            ...
            if event["event_type"] == "BOOK_SNAPSHOT":
                drift_metrics = self._process_snapshot(event, order_book)
    ...
    def _process_snapshot(
        self, event: dict[str, Any], order_book: OrderBookState
    ) -> dict[str, float] | None:
        ...
        if not order_book.initialized:
            ...
            drift_metrics = self.drift_tracker.calculate_drift(order_book, event)

            if (
                self.resync_on_drift
                and drift_metrics["rms_error"] > self.drift_threshold
            ):
                logger.warning(
    ...
    def _process_trade(self, event: dict[str, Any], order_book: OrderBookState) -> None:
        ...
        if all([trade_price, trade_quantity, trade_side]):
            order_book.apply_trade(
                {"price": trade_price, "quantity": trade_quantity, "side": trade_side}
            )

    def _process_delta(self, event: dict[str, Any], order_book: OrderBookState) -> None:
        ...
        if all(
            [
                delta_side is not None,
                delta_price is not None,
