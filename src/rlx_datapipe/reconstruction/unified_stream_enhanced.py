...
class UnifiedEventStreamEnhanced(UnifiedEventStream):
    ...
    def __init__(
        self,
        symbol: str,
        config: EnhancedUnificationConfig | None = None,
    ):
        """
    ...
    def _process_streaming_mmap(
        ...
        book_snapshots_path: Path | None,
        book_deltas_path: Path | None,
        ...
        if book_deltas_path:
            ...
            def process_delta_chunk(chunk: pl.DataFrame) -> pl.DataFrame:
                ...
                sorted_chunk, recovery_needed = self.delta_processor.process_batch(
                    chunk
                )
        ...
        for event_type in unified_df["event_type"].unique():
            ...
            if event_type == "BOOK_SNAPSHOT":
                ...
                if output_path:
                    ...
                    enriched_df.write_parquet(output_path / "enriched_deltas.parquet")
        ...
        for symbol in snapshot_df["symbol"].unique():
            ...
            for snapshot_time in symbol_snapshots["origin_time"].unique():
                ...
                if self.order_book_engine.last_update_id == 0:
                    ...
                    logger.info(
                        f"Initialized order book from snapshot at {snapshot_time}"
                    )
                else:
                    # Calculate drift
                    ...
                    if (
                        drift.get("total_drift", 0) > 1000000
                    ):  # Threshold in scaled units
                        logger.warning("High drift detected, resyncing from snapshot")
                        self.order_book_engine.process_snapshot(snapshot)
    ...
    def get_pending_queue_stats(self) -> dict:
        ...
        return {
            ...
            "usage_percent": (len(self.pending_queue) / self.pending_queue.maxlen)
            * 100,
        }
