...
class UnifiedEventStreamWithReplay(UnifiedEventStreamEnhanced):
    ...
    def _process_batch_with_replay(
        ...
        stats = {
            ...
            "trades_processed": len(
                replayed_df.filter(pl.col("event_type") == "TRADE")
            ),
            "snapshots_processed": len(
                replayed_df.filter(pl.col("event_type") == "BOOK_SNAPSHOT")
            ),
            "deltas_processed": len(
                replayed_df.filter(pl.col("event_type") == "BOOK_DELTA")
            ),
            "events_with_book_state": len(
                replayed_df.filter(pl.col("top_bid").is_not_null())
            ),
        }
        ...
        for event_type, data_path in data_paths:
            ...
            for batch_num, chunk in enumerate(
                ...
                if output_path:
                    batch_file = (
                        output_path
                        / f"{event_type.lower()}_batch_{batch_num:04d}.parquet"
                    )
