...
from rlx_datapipe.common.decimal_utils import PRICE_SCALE, QUANTITY_SCALE
...
def update_bounded_levels(
    ...
    if is_bid and insert_pos < max_levels:
    ...
    elif not is_bid and insert_pos < max_levels:
        # Shift elements, losing the worst
        for i in range(max_levels - 1, insert_pos, -1):
            prices[i] = prices[i - 1]
            quantities[i] = quantities[i - 1]
...
def apply_delta_batch_optimized(
    ...
    for i in range(num_deltas):
        ...
        if is_bid:
            ...
            ask_count, _ = update_bounded_levels(
                ask_prices, ask_quantities, ask_count, max_levels,
    ...
    return (
        bid_count, ask_count,
        bid_top_prices, bid_top_quantities,
        ask_top_prices, ask_top_quantities
    )
