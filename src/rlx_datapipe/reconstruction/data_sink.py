...
class DataSinkConfig:
    ...
    max_file_size_mb: int = (
        DEFAULT_MAX_FILE_SIZE_MB  # Target file size between 100-500MB
    )
...
class DataSink:
        ...
        try:
            while True:
                ...
                if (
                    ...
                    or self.estimated_batch_memory >= self.max_memory_bytes
                ):

                    if self.estimated_batch_memory >= self.max_memory_bytes:
                        logger.warning(
                            f"Writing batch due to memory limit: {self.estimated_batch_memory / 1024 / 1024:.1f}MB"
                        )
            ...
            result.update(
                {
                    "bids": (
                        json.dumps(
                            [[str(price), str(qty)] for price, qty in event.bids]
                        )
                        if event.bids
                        else None
                    ),
                    "asks": (
                        json.dumps(
                            [[str(price), str(qty)] for price, qty in event.asks]
                        )
                        if event.asks
                        else None
                    ),
    ...
    async def _write_partition(
        ...
        if (
            current_size > 0
            and current_size + estimated_new_size > self.max_file_size_bytes
        ):
            ...
            logger.info(
                f"Partition {partition_key} would exceed size limit ({current_size:,} + {estimated_new_size:,} > {self.max_file_size_bytes:,}), creating file #{file_count}"
            )
        else:
        ...
        self.partition_size_bytes[partition_path_str] = (
            self.partition_size_bytes.get(partition_path_str, 0) + file_size
        )
        ...
        logger.debug(
            f"Wrote {len(events)} events to {file_path} (size: {file_size:,} bytes)"
        )
    ...
    def _create_arrow_table(self, events: list[dict[str, Any]]) -> pa.Table:
        ...
        arrays.append(
            pa.array([e["trade_price"] for e in events], type=pa.decimal128(38, 18))
        )
        arrays.append(
            pa.array([e["trade_quantity"] for e in events], type=pa.decimal128(38, 18))
        )
        ...
        arrays.append(
            pa.array([e["delta_price"] for e in events], type=pa.decimal128(38, 18))
        )
        arrays.append(
            pa.array([e["delta_quantity"] for e in events], type=pa.decimal128(38, 18))
        )
    ...
    def _cleanup_temp_files(self) -> None:
        ...
        if temp_files:
            logger.warning(
                f"Found {len(temp_files)} orphaned temp files, cleaning up..."
            )
