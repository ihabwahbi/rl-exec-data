...
class SchemaNormalizer:
    ...
    def _add_snapshot_fields(
        self, unified: dict[str, Any], event: dict[str, Any]
    ) -> None:
    ...
    def _normalize_book_levels(
        self, levels: list[list[Any]] | list[dict[str, Any]]
    ) -> list[list[Decimal]]:
        ...
        for level in levels:
            if isinstance(level, list) and len(level) >= 2:
                ...
                price = ensure_decimal128(level.get("price", level.get("p", 0)))
    ...
    def _apply_updates_to_levels(
        self, levels: list[list[Decimal]], updates: list[dict[str, Decimal]]
    ) -> list[list[Decimal]]:
