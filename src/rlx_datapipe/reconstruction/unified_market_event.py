...
class UnifiedMarketEvent:
    ...
    trade_side: str | None = None  # 'BUY' | 'SELL'

    # Book snapshot fields (null if not BOOK_SNAPSHOT)
    bids: list[tuple[Decimal, Decimal]] | None = None  # [(price, quantity), ...]
    asks: list[tuple[Decimal, Decimal]] | None = None  # [(price, quantity), ...]
    is_snapshot: bool | None = None

    # Book delta fields (null if not BOOK_DELTA)
    delta_side: str | None = None  # 'BID' | 'ASK'
    ...
    def __post_init__(self):
        ...
        if self.event_type == "TRADE":
            if (
                ...
                or self.trade_price is None
                or self.trade_quantity is None
            ):
                raise ValueError(
                    "TRADE events must have trade_id, trade_price, and trade_quantity"
                )

        elif self.event_type == "BOOK_SNAPSHOT":
            if self.bids is None or self.asks is None or self.is_snapshot is None:
                raise ValueError(
                    "BOOK_SNAPSHOT events must have bids, asks, and is_snapshot"
                )

        elif self.event_type == "BOOK_DELTA":
            if (
                self.delta_side is None
                or self.delta_price is None
                or self.delta_quantity is None
            ):
                raise ValueError(
                    "BOOK_DELTA events must have delta_side, delta_price, and delta_quantity"
                )
