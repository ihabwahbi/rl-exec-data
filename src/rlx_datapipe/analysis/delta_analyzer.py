...
class SequenceGapAnalyzer:
    ...
    def analyze_gaps(self, update_ids: List[int]) -> Dict[str, Any]:
        ...
        if gaps:
            ...
            self.gap_ratio_percent = (actual_gaps / total_possible_gaps) * 100 if total_possible_gaps > 0 else 0.0
...
class MemoryProfiler:
    ...
    def record_memory(self) -> float:
        ...
        if memory_gb > self.limit_gb * 0.9:
            logger.warning(f"Memory usage {memory_gb:.2f}GB approaching limit of {self.limit_gb}GB")
    ...
    def get_memory_stats(self) -> Dict[str, float]:
        ...
        p95_gb = sorted(samples_gb)[int(len(samples_gb) * 0.95)] if len(samples_gb) > 0 else 0.0
...
def create_sample_delta_data(num_events: int = 1000) -> pl.DataFrame:
    ...
    data = {
        ...
        "origin_time": [int(time.time() * 1e9) + i * 1000000 for i in range(num_events)],
        "side": [random.choice(["bid", "ask"]) for _ in range(num_events)],
        "price": [round(45000 + random.uniform(-1000, 1000), 2) for _ in range(num_events)],
    ...
    if num_events > 100:
        ...
        gap_positions = random.sample(range(10, num_events - 10), min(5, num_events // 100))
