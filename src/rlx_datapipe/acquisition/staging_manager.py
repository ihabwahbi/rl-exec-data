...
class DataStagingManager:
    ...
    def get_files_by_status(self, status: FileStatus) -> list[FileManifest]:
        ...
        """
        return [
            manifest for manifest in self.manifest.values() if manifest.status == status
        ]
    ...
    def generate_readiness_certificate(
        ...
        for data_type in required_data_types:
            ...
            if not ready_files:
                ...
                certificate["issues"].append(
                    f"{data_type}: Coverage starts late ({type_status[data_type]['date_range']['start']})"
                )
            elif type_status[data_type]["date_range"]["end"] < end_date:
                certificate["issues"].append(
                    f"{data_type}: Coverage ends early ({type_status[data_type]['date_range']['end']})"
                )
        ...
        cert_file = (
            self.staging_root
            / f"readiness_certificate_{symbol}_{start_date}_{end_date}.json"
        )
        ...
        if certificate["ready"]:
            logger.info(
                f"✅ Data is READY for {symbol} from {start_date} to {end_date}"
            )
        else:
            logger.warning(
                f"❌ Data NOT ready for {symbol}: {len(certificate['issues'])} issues"
            )
