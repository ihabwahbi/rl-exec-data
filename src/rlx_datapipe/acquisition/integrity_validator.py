...
class IntegrityValidator:
    ...
    def validate_file(
        self, file_path: Path, data_type: str, expected_checksum: str | None = None
    ) -> ValidationResult:
        ...
        if not checks["file_exists"]:
            ...
            return ValidationResult(
                file_path, False, checks, errors, warnings, metadata
            )
        ...
        try:
            ...
            errors.append(f"Failed to read Parquet file: {e!s}")
            return ValidationResult(
                file_path, False, checks, errors, warnings, metadata
            )
        ...
        if data_type in self.expected_schemas:
            ...
            for col in schema.get("numeric_columns", []):
                if col in df.columns:
                    ...
                    checks.update(
                        {f"{col}_{k}": v for k, v in numeric_checks["checks"].items()}
                    )
            ...
            for col in schema.get("positive_columns", []):
                if col in df.columns:
                    ...
                    checks.update(
                        {f"{col}_{k}": v for k, v in positive_checks["checks"].items()}
                    )
    ...
    def _validate_origin_time(self, df: pl.DataFrame) -> dict[str, Any]:
        ...
        if not col.is_empty():
            ...
            try:
                ...
                checks["reasonable_time_range"] = (
                    year_2020 <= min_dt <= year_2030
                    and year_2020 <= max_dt <= year_2030
                )
        ...
        if "update_id" in df.columns:
            ...
            if len(update_ids) > 1:
                ...
                if large_gap_count > 0:
                    warnings.append(
                        f"Found {large_gap_count} very large update_id gaps (>1000)"
                    )
    ...
    def validate_date_continuity(
        self, file_paths: list[Path], expected_start: datetime, expected_end: datetime
    ) -> ValidationResult:
        ...
        if not file_paths:
            ...
            return ValidationResult(
                Path("multiple"), False, checks, errors, warnings, metadata
            )
        ...
        if not checks["covers_start"]:
            errors.append(
                f"Coverage starts {coverage_start}, expected {expected_start}"
            )
        ...
        checks["no_missing_days"] = (
            len(unique_dates) >= expected_days * 0.95
        )  # Allow 5% missing
        ...
        return ValidationResult(
            Path("multiple"), passed, checks, errors, warnings, metadata
        )
