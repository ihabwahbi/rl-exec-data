...
class DataDownloader:
    ...
    def download_file(self, s3_key: str, local_path: Optional[Path] = None) -> Path:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.info(f"Downloading {s3_key} (attempt {attempt + 1}/{self.max_retries + 1})")
                
                # Download with progress bar
                with tqdm(total=file_size, 
                         unit='B', 
                         unit_scale=True, 
                         desc=s3_key.split('/')[-1]) as pbar:
                ...
                logger.warning(f"Download attempt {attempt + 1} failed for {s3_key}: {e}")
                ...
                if attempt < self.max_retries:
                    ...
                    # Final failure
                    self.stats['failed'] += 1
                    logger.error(f"❌ Failed to download {s3_key} after {self.max_retries + 1} attempts")
                    raise
    ...
    async def download_date_range(self, 
                                 ...
                                 data_type: str,
                                 start_date: datetime,
                                 end_date: datetime) -> Tuple[List[Path], List[Dict[str, Any]]]:
        ...
        logger.info(f"Finding files for {symbol} {data_type} from {start_date.date()} to {end_date.date()}")
        ...
        async def download_chunk(chunk: DownloadChunk) -> Optional[Path]:
        ...
        logger.info(f"Downloaded {mb_downloaded:.1f} MB in {duration:.1f}s ({speed_mbps:.1f} MB/s)")
        
        return successful, failed
        
    async def _download_chunk_async(self, chunk: DownloadChunk) -> Optional[Path]:
    ...
    def download_batch(self, 
                      ...
                      data_types: List[str],
                      start_date: datetime,
                      end_date: datetime) -> Dict[str, Tuple[List[Path], List[Dict[str, Any]]]]:
        ...
        for data_type in data_types:
            ...
            try:
                ...
                results[data_type] = ([], [{'key': 'all', 'error': str(e), 'retries': 0}])
    ...
    def get_download_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': self.stats['downloaded'] / self.stats['total_files'] if self.stats['total_files'] > 0 else 0,
