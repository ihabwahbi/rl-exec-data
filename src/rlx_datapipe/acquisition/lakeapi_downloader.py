...
class LakeAPIDownloader:
    ...
    def generate_download_tasks(self,
                              ...
                              chunk_days: int = 7) -> List[DownloadTask]:
        ...
        while current_date <= end_date:
            ...
            for data_type in data_types:
                ...
                date_str = f"{current_date.strftime('%Y%m%d')}_{chunk_end.strftime('%Y%m%d')}"
    ...
    async def download_task(self, task: DownloadTask) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Date range: {task.start_date.date()} to {task.end_date.date()}")
            ...
            if result['success']:
                ...
                logger.info(f"✅ Downloaded {task.rows_downloaded} rows "
    ...
    async def download_with_retry(self, task: DownloadTask) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            if attempt > 0:
                ...
                logger.info(f"Retrying {task.data_type} download in {delay:.1f}s (attempt {attempt + 1})")
    ...
    async def download_batch(self, tasks: List[DownloadTask], dry_run: bool = False) -> Dict[str, Any]:
        ...
        with tqdm(total=len(tasks), desc="Downloading", unit="files") as pbar:
            ...
            results = await asyncio.gather(
                *[download_and_update(task) for task in tasks],
        ...
        avg_speed_mbps = (self.stats['total_bytes'] / (1024 * 1024)) / duration if duration > 0 else 0
        ...
        logger.info(f"Success rate: {self.stats['completed']}/{len(tasks)} "
                   f"({100 * self.stats['completed'] / len(tasks):.1f}%)")
        logger.info(f"Total data: {summary['total_size_mb']:.1f} MB "
    ...
    async def _dry_run_batch(self, tasks: List[DownloadTask]) -> Dict[str, Any]:
        ...
        for task in sample_tasks:
            try:
                ...
                if estimate.get('estimated_rows', 0) > 0:
                    ...
                    logger.info(f"Estimate for {task.data_type}: "
        ...
        logger.info(f"DRY RUN ESTIMATE:")
        logger.info(f"  Tasks: {len(tasks)}")
        logger.info(f"  Estimated data: {total_estimated_mb:.1f} MB ({total_estimated_rows:,} rows)")
