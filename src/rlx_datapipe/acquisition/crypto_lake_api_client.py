...
class CryptoLakeAPIClient:
    ...
    def list_available_data(
        ...
        try:
            ...
            end_date = datetime.now() - timedelta(
                days=1
            )  # Data available until yesterday
            ...
            for data_type in data_types:
                try:
                    logger.info(
                        f"Checking {data_type} availability for {symbol} on {exchange}"
                    )
                    ...
                    availability[data_type] = {
                        ...
                        "columns": (
                            list(sample_data.columns) if len(sample_data) > 0 else []
                        ),
                        "date_range_tested": {
    ...
    def download_data(
        ...
        try:
            ...
            logger.info(
                f"✅ Downloaded {len(data)} rows ({file_size / 1024 / 1024:.1f} MB)"
            )
            ...
            return {"success": False, "error": str(e), "rows": 0, "file_path": None}
    ...
    def estimate_data_size(
        ...
        try:
            ...
            return {"estimated_rows": 0, "estimated_size_mb": 0, "error": str(e)}
