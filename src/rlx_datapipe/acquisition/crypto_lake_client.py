...
class CryptoLakeClient:
    ...
    def __init__(self, region_name: str = "us-east-1", bucket_name: str | None = None):
    ...
    def _discover_bucket_name(self) -> str:
        ...
        try:
            ...
            if not buckets:
                raise ValueError(
                    "No accessible buckets found with provided credentials"
                )
    ...
    def list_available_data(
        self, symbol: str = "BTC-USDT", exchange: str = "binance"
        ...
        book_files = [
            f
            for f in all_files
            if "book" in f["key"].lower() and "delta" not in f["key"].lower()
        ]
        delta_files = [
            f
            for f in all_files
            if "book_delta" in f["key"].lower() or "delta" in f["key"].lower()
        ]

        logger.info(
            f"Found {len(trades_files)} trades files, {len(book_files)} book files, {len(delta_files)} delta files"
        )
    ...
    def list_files_in_date_range(
        ...
        # Sort by date
        filtered_files.sort(
            key=lambda x: self._extract_date_from_key(x["key"]) or datetime.min
        )

        return filtered_files

    def _extract_date_from_key(self, key: str) -> datetime | None:
