...
class KSValidator(BaseValidator):
    ...
    def _validate(self, sample1: np.ndarray, sample2: np.ndarray) -> tuple[bool, dict]:
        ...
        logger.info(
            f"K-S test: statistic={statistic:.4f}, p_value={p_value:.4f}, passed={passed}"
        )
...
class PowerLawValidator(BaseValidator):
    ...
    def _validate(self, trade_sizes: np.ndarray, _: Any = None) -> tuple[bool, dict]:
        ...
        if len(trade_sizes) < 100:
            raise ValueError(
                f"Insufficient data: {len(trade_sizes)} positive trade sizes (need at least 100)"
            )
        ...
        logger.info(
            f"Power law fit: alpha={alpha:.3f}, xmin={xmin:.6f}, passed={passed}"
        )
...
class BasicStatsCalculator(BaseValidator):
    ...
    def _validate(self, data1: np.ndarray, data2: np.ndarray) -> tuple[bool, dict]:
        ...
        mean_diff = abs(stats1["mean"] - stats2["mean"]) / max(
            abs(stats1["mean"]), 1e-10
        )
        std_diff = abs(stats1["std"] - stats2["std"]) / max(abs(stats1["std"]), 1e-10)
        median_diff = abs(stats1["median"] - stats2["median"]) / max(
            abs(stats1["median"]), 1e-10
        )
        ...
        metrics = {
            ...
            "interpretation": (
                ...
                if passed
                else "Statistics differ significantly"
            ),
        }

        logger.info(
            f"Basic stats: mean_diff={mean_diff:.4f}, std_diff={std_diff:.4f}, passed={passed}"
        )
