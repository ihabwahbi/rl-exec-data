...
class GoldenSampleLoader:
                    ...
                    try:
                        ...
                        if "stream" in msg:
                            ...
                            if len(stream_parts) > 1:
                                ...
                                self._message_counts[stream_type] = (
                                    self._message_counts.get(stream_type, 0) + 1
                                )
    ...
    def extract_trades(
        self, filepath: Path, start_ns: int | None = None, end_ns: int | None = None
    ) -> Iterator[np.ndarray]:
    ...
    def load_all_prices(
        self, filepath: Path, message_type: str = "trade"
    ) -> np.ndarray:
