    ...
    def run(self,
            ...
            try:
                with open(checkpoint_file, 'r') as f:
                    ...
                    if content:
                        ...
                        self._checkpoints = {}
                logger.info(f"Loaded checkpoint with {len(self._checkpoints)} completed validators")
        ...
        for validator in self.validators:
            ...
            try:
                ...
                if hasattr(validator, '_requires_full_data') and validator._requires_full_data:
                    # Load full datasets for validators that need them
                    logger.info("  Loading full datasets...")
                    if "trade" in validator.name.lower() or "power" in validator.name.lower():
                        golden_data = golden_loader.load_all_trades(golden_sample_path)
                        comparison_data = comparison_loader.load_all_trades(comparison_path)
                    else:
                        golden_data = golden_loader.load_all_prices(golden_sample_path)
                        comparison_data = comparison_loader.load_all_prices(comparison_path)
                else:
